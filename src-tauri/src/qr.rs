...
fn parse_migration_url(url: &str) -> Result<Vec<TokenData>, String> {
    ...
    let parsed_url = url::Url::parse(url)
        .map_err(|e| format!("Failed to parse migration URL: {}", e))?;
    
    // Get the data parameter
    let data_param = parsed_url.query_pairs()
        .find(|(key, _)| key == "data")
        .map(|(_, value)| value.to_string())
...
fn parse_migration_payload(data: &[u8]) -> Result<Vec<TokenData>, String> {
    ...
    while i < data.len() {
        if data[i] == 0x0a {  // Field 1 (OtpParameters)
            i += 1;
...
fn parse_otp_parameter(data: &[u8]) -> Result<TokenData, String> {
    ...
    while i < data.len() {
        ...
        match field {
            0x0a => { // Field 1: secret (bytes)
                let (length, bytes_read) = decode_varint(&data[i..])?;
                i += bytes_read;
                secret_bytes = Some(data[i..i + length].to_vec());
                i += length;
            }
            0x12 => { // Field 2: name (string)
                let (length, bytes_read) = decode_varint(&data[i..])?;
                i += bytes_read;
